class OrderFrequenciesController < ApplicationController
  ...
  def new
    ...
    @frequency_unit = FrequencyUnit.all
  end
  ...
  def create
    ...
    respond_to do |format|
      if @order_frequency.save
        format.html { redirect_to :order_frequencies, notice: 'Order frequency was successfully created.' }
      else
  ...
  def update
    respond_to do |format|
      if @order_frequency.update(order_frequency_params)
        format.html { redirect_to :order_frequencies, notice: 'Order frequency was successfully updated.' }
      else
  ...
  private
    ...
    def order_frequency_params
      params.fetch(:order_frequency).permit(:value, :unit)
    end
end
